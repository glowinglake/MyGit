"""
PPO (Proximal Policy Optimization) for CarRacing-v3 (Discrete Actions)
...
Run:
  python carracing_ppo.py --episodes 1000
  python carracing_ppo.py --load carracing_ppo.pth --eval-episodes 5
"""
...
class RolloutBuffer:
    ...
    def __init__(self, buffer_size, num_envs, state_shape, device):
...
class PPOAgent:
    ...
    def __init__(
        ...
        # Network
        self.network = ActorCritic(input_channels, action_dim).to(self.device)
    ...
    def update(self, buffer, ppo_epochs=4, batch_size=128):
        ...
        - actions:       (B,)         - discrete action indices
        ...
        for epoch in range(ppo_epochs):
            ...
            for states, actions, old_log_probs, advantages, returns in buffer.get_batches(batch_size):
                # states: (B, C, H, W), actions: (B,), old_log_probs: (B,)
...
def main():
    ...
    parser.add_argument("--target-kl", type=float, default=0.015, help="KL threshold for early stopping (None to disable)")
    ...
    # ===== TRAINING MODE =====
    print("=" * 60)
    print("PPO for CarRacing-v3 (Discrete Actions)")
    print("=" * 60)
    ...
    buffer = RolloutBuffer(args.rollout_steps, num_envs, state_shape, agent.device)
    ...
    while global_step < args.total_timesteps:
        ...
        # KL threshold annealing: start permissive (0.02), end conservative (0.005)
